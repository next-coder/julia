...
(define (renumber-labels! body labelmap)
  (let loop ((stmts (cdr body)))
    (if (pair? stmts)
        (let ((el (car stmts)))
          (if (pair? el)
              (case (car el)
                ((label goto enter) (set-car! (cdr el)  (get labelmap (cadr el)  (cadr el))))
                ((gotoifnot)        (set-car! (cddr el) (get labelmap (caddr el) (caddr el))))
                (else #f)))
          (loop (cdr stmts))))))

;; make label numbers equal their indices
(define (labels-to-indices! body)
  (let ((tbl (table)))
    (let loop ((stmts (cdr body))
               (i     1))
      (if (pair? stmts)
          (let ((el (car stmts)))
            (if (and (pair? el) (eq? (car el) 'label))
                (put! tbl (cadr el) i))
            (loop (cdr stmts) (+ i 1)))))
    (renumber-labels! body tbl)
    body))

;; merge adjacent labels
(define (combine-labels! body)
  (let ((tbl (table)))
    (let loop ((stmts (cdr body)))
      (if (pair? stmts)
          (let ((el  (car stmts))
            ...
            (if (and (pair? el) (eq? (car el) 'label)
                     ...
                     (pair? nxt) (pair? (car nxt)) (eq? (caar nxt) 'label))
                (begin (put! tbl (cadr (car nxt)) (cadr el))
                       (set-cdr! stmts (cdr nxt))
                       (loop stmts))
                (loop nxt)))))
    (renumber-labels! body tbl)
    body))
...
(define (renumber-lambda lam)
  (combine-labels! (lam:body lam))
  (labels-to-indices! (lam:body lam))
  (define nssavalues 0)
